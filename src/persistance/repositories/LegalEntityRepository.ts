...
@injectable()
export class LegalEntityRepository implements IBaseRepository<LegalEntity> {
  ...
  async create(item: LegalEntity): Promise<LegalEntity> {
    const [created] = await this.db.insert(legalEntity).values(item).returning()
    return created
  }
  async update(id: string, item: LegalEntity): Promise<LegalEntity | null> {
